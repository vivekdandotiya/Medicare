<x-app-layout>
    <div class="max-w-3xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <div class="mb-6">
            <a href="{{ route('brands.index') }}" class="text-teal-600 hover:text-teal-700 font-semibold text-sm flex items-center gap-1">
                &larr; Back to Brands List
            </a>
            <h1 class="text-3xl font-extrabold text-gray-900 mt-2">Edit Brand</h1>
            <p class="text-gray-500 text-sm">Update details for the brand</p>
        </div>
        ...
        <form action="{{ route('brands.update', $brand) }}"
            ...
            <div class="mb-6">
                ...
                @if($brand->logo)
                    <div class="mt-2 flex items-center gap-2">
                        ...
                        <img src="{{ asset($brand->logo) }}" class="w-10 h-10 object-cover rounded-lg border border-gray-100">
                    </div>
                @endif
            </div>
            ...
            <div class="mb-6">
                ...
                <textarea name="description"
                          rows="4"
                          class="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:border-teal-500 focus:ring-teal-500">{{ old('description', $brand->description) }}</textarea>
            </div>
